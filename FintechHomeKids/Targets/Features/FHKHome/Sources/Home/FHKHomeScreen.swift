        ...
        FHKScreenContainer {
            VStack {
                ...
                .fullScreenCover(isPresented: $showPermissions) {
                    FHKPermissionView(provider: viewModel.fhkCameraPermission)
                }
...
private extension FHKHomeScreen {
    
    func getMemberLoaded(member: FHKMemberEntity) -> FHKMemberItem {
        FHKMemberItem(id: viewModel.getId(member: member),
                      ...
                      action: { _ in
            router.navigate(to: .memberDetail(member.id))
        })
    }
    
    func getRewardCollectCardLoaded(ticket: FHKRewardCollectedEntity) -> FHKRewardCollectCard {
